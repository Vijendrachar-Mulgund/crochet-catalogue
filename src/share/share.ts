// share — builds the branded, print-optimised catalogue view for Save-as-PDF.
...
function productDetails(p: Product, showPrices: boolean): string {
  const rows: string[] = [];
  if (showPrices && p.price != null && (p.price as unknown) !== '') {
  ...
  if (p.madeToOrder) {
    specs.push(
      '<b>Made to order</b>' +
        (p.productionTime ? ' (' + esc(p.productionTime) + ')' : ''),
    );
  }
  if (specs.length) {
...
export function buildHtml(options: ShareOptions): string {
  ...
  const byCat = new Map<
    number | null,
    { cat: { id: number | null; name: string }; items: Product[] }
  >();
  ...
  const cover =
    ...
    (!showPrices ? '<div class="ph-note">Prices available on request</div>' : '') +
    '</div></header>';
...
export function printShare(options: ShareOptions): void {
  const area = document.getElementById('print-area');
  if (area) area.innerHTML = buildHtml(options);
